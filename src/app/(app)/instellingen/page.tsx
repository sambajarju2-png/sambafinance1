...
import { User, Mail, BellRing, Wallet, HelpCircle, LogOut, ChevronRight, Loader2, Trophy, Trash2 } from 'lucide-react';
...
function BudgetSetting({ t }: { t: ReturnType<typeof useTranslations> }) {
    ...
    <div className="rounded-card border border-pw-border bg-pw-surface p-4">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={handleSave} disabled={saving}
          ...
          {saving ? <Loader2 className="h-3 w-3 animate-spin" strokeWidth={2} /> : saved ? t('saved') : t('save')}
        </button>
      </div>
    </div>
  );
