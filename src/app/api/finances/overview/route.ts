...
export async function GET(req: NextRequest) {
  ...
  if (!finances) {
    return NextResponse.json({
      has_finances: false,
      totaal_inkomen: 0,
      totaal_vaste_lasten: 0,
      totaal_open_rekeningen: 0,
      vrij_besteedbaar: 0,
      expenses_count: 0,
      bills_count: 0,
      toeslagen: null,
  ...
  const totaal_inkomen = finances.netto_inkomen +
    (finances.partner_inkomen || 0) +
    (finances.duo_inkomen || 0) +
    (finances.uitkering_inkomen || 0) +
    (finances.toeslagen_inkomen || 0) +
    (finances.overig_inkomen || 0);

  // Total monthly vaste lasten
  // Exclude expenses that have a linked unpaid bill (prevents double counting)
  const linkedExpenseIds = new Set(
    bills.filter((b: { expense_id: string | null }) => b.expense_id).map((b: { expense_id: string }) => b.expense_id)
  );

  const totaal_vaste_lasten = expenses.reduce((sum: number, exp: { id: string; monthly_amount: number }) => {
    if (linkedExpenseIds.has(exp.id)) return sum; // skip — counted in bills
    return sum + (exp.monthly_amount || 0);
  }, 0);

  // Total open bills (already includes linked expenses that went to incasso)
  ...
  // Expenses in incasso (linked to a bill)
  const expenses_in_incasso = (expenses as Array<{ id: string; name: string; monthly_amount: number }>)
    ...
    .map(exp => ({
      ...
      bill: (bills as Array<{ id: string; expense_id: string | null; amount: number; vendor: string }>).find(b => b.expense_id === exp.id),
    }));
  ...
  return NextResponse.json({
    ...
    salary_window: finances.salary_day_from && finances.salary_day_to
      ? { from: finances.salary_day_from, to: finances.salary_day_to }
      : null,
  });
}
