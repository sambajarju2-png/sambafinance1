...
export async function GET() {
  ...
  try {
    ...
    const { data: settings } = await supabase
      .from('user_settings')
      .select('display_name, first_name, last_name, date_of_birth, gemeente, language, dark_mode, notify_push_enabled, notify_email_welcome, notify_email_features, notify_email_digest, notify_community_enabled, monthly_budget_cents, budgets, dashboard_modules')
      .eq('user_id', userId)
      .single();
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (typeof body.notify_community_enabled === 'boolean') updates.notify_community_enabled = body.notify_community_enabled;
    ...
    if (body.dashboard_modules && typeof body.dashboard_modules === 'object') updates.dashboard_modules = body.dashboard_modules;
