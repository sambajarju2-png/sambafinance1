    ...
    <AppShell billCount={bills.length} bills={displayBills} userName={userName} userEmail={userEmail} onSignOut={signOut}>
      {({ activeView, household, searchQuery }) => {
        ...
        if (!seeded && !loading) {
          return (
            <div className="flex flex-col items-center justify-center py-20 text-center px-4">
              <div className="w-14 h-14 rounded-xl bg-brand-blue-pale flex items-center justify-center mb-4"><Database className="w-7 h-7 text-brand-blue" /></div>
              <h2 className="text-[16px] font-bold text-navy mb-2">Database is leeg</h2>
              <p className="text-[13px] text-muted max-w-[360px] mb-5">Je database is verbonden maar bevat nog geen data. Klik hieronder om de demo-gegevens te laden.</p>
              <button onClick={seed} className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-navy text-white text-[13px] font-bold hover:bg-navy-light transition-colors"><Database className="w-4 h-4" /> Seed demo data</button>
            </div>
          )
        }
