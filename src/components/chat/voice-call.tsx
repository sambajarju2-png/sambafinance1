      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[15px] font-semibold text-gray-900 dark:text-white">
                {pick(lang, { nl: 'Gesprek afgerond', en: 'Call ended', pl: 'Rozmowa zakończona', tr: 'Görüşme bitti', fr: 'Appel terminé', ar: 'انتهت المكالمة' })}
              </p>
        ...
        <div className="mb-5 grid grid-cols-3 gap-3">
          <div className="rounded-xl bg-gray-50 dark:bg-white/5 px-3 py-3 text-center">
            ...
            <p className="text-[11px] text-gray-500">{pick(lang, { nl: 'Toegevoegd', en: 'Added', pl: 'Dodano', tr: 'Eklendi', fr: 'Ajouté', ar: 'تمت الإضافة' })}</p>
          </div>
          <div className="rounded-xl bg-gray-50 dark:bg-white/5 px-3 py-3 text-center">
            <p className="text-[20px] font-bold text-pw-blue">{data.sentToChat}</p>
            <p className="text-[11px] text-gray-500">{pick(lang, { nl: 'In chat', en: 'To chat', pl: 'Do czatu', tr: 'Sohbete', fr: 'Vers le chat', ar: 'إلى المحادثة' })}</p>
          </div>
          <div className="rounded-xl bg-gray-50 dark:bg-white/5 px-3 py-3 text-center">
            <p className="text-[20px] font-bold text-gray-600 dark:text-gray-300">{data.transcriptCount}</p>
            <p className="text-[11px] text-gray-500">{pick(lang, { nl: 'Berichten', en: 'Messages', pl: 'Wiadomości', tr: 'Mesajlar', fr: 'Messages', ar: 'الرسائل' })}</p>
          </div>
        </div>
        ...
        {data.billsAdded.length > 0 && (
          <div className="mb-5">
            <p className="mb-2 text-[12px] font-medium text-gray-500 uppercase tracking-wide">
              {pick(lang, { nl: 'Rekeningen toegevoegd', en: 'Bills added', pl: 'Dodane rachunki', tr: 'Eklenen faturalar', fr: 'Factures ajoutées', ar: 'الفواتير المضافة' })}
            </p>
        ...
        <div className="space-y-2.5">
          {data.billsAdded.length > 0 && (
            ...
            >
              {pick(lang, { nl: 'Bekijk rekeningen', en: 'View bills', pl: 'Zobacz rachunki', tr: 'Faturaları gör', fr: 'Voir les factures', ar: 'عرض الفواتير' })}
          ...
          >
            {pick(lang, { nl: 'Sluiten', en: 'Close', pl: 'Zamknij', tr: 'Kapat', fr: 'Fermer', ar: 'إغلاق' })}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-gradient-to-b from-pw-navy to-[#0F1B2D]">
      ...
      <div className="flex flex-1 flex-col items-center justify-center">
        ...
        <p className="mt-2 text-lg font-semibold text-white">
          {status === 'connecting' && (pick(lang, { nl: 'Verbinden...', en: 'Connecting...', pl: 'Łączenie...', tr: 'Bağlanılıyor...', fr: 'Connexion...', ar: 'جارٍ الاتصال...' }))}
          {isActive && isSpeaking && 'PayBuddy'}
          {isActive && !isSpeaking && (pick(lang, { nl: 'Luistert...', en: 'Listening...', pl: 'Słucha...', tr: 'Dinliyor...', fr: 'Écoute...', ar: 'يستمع...' }))}
        ...
        {errorDetail && (
          <div className="mx-6 mt-4 max-w-sm rounded-2xl bg-pw-red/8 border border-pw-red/15 px-4 py-3">
            ...
            <button onClick={() => navigator.clipboard.writeText(errorDetail)} className="mt-2 text-[10px] text-pw-red/40 underline">
              {pick(lang, { nl: 'Kopieer fout', en: 'Copy error', pl: 'Kopiuj błąd', tr: 'Hatayı kopyala', fr: "Copier l'erreur", ar: 'نسخ الخطأ' })}
            </button>
          </div>
        )}
      </div>
      ...
      <input
        ...
        onChange={async (e) => {
          ...
          try {
            ...
            const data = await res.json();
            const spoken: string = data.spoken || (pick(lang, { nl: 'Ik heb de foto bekeken maar kon geen details vinden.', en: 'I looked at the photo but could not find details.', pl: 'Obejrzałem zdjęcie, ale nie znalazłem szczegółów.', tr: 'Fotoğrafa baktım ama ayrıntı bulamadım.', fr: "J'ai regardé la photo mais je n'ai pas trouvé de détails.", ar: 'نظرت إلى الصورة لكنني لم أجد تفاصيل.' }));
        ...
        <div className="absolute inset-x-0 bottom-28 flex justify-center z-10 px-6">
          <div className="flex items-center gap-3 rounded-2xl bg-white/10 backdrop-blur-md border border-white/10 px-5 py-3">
            ...
            <p className="text-[13px] text-white/80">
              {pick(lang, { nl: 'Foto analyseren...', en: 'Analysing photo...', pl: 'Analiza zdjęcia...', tr: 'Fotoğraf analiz ediliyor...', fr: 'Analyse de la photo...', ar: 'جارٍ تحليل الصورة...' })}
            </p>
          </div>
        </div>
      )}
          ...
          <div className="w-full max-w-xs rounded-2xl bg-white/10 backdrop-blur-md border border-white/10 p-4 text-center">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {pick(lang, { nl: 'Camera', en: 'Camera', pl: 'Aparat', tr: 'Kamera', fr: 'Caméra', ar: 'الكاميرا' })}
              </button>
              <button
                onClick={() => {
                  if (photoInputRef.current) {
              ...
              >
                {pick(lang, { nl: 'Galerij', en: 'Gallery', pl: 'Galeria', tr: 'Galeri', fr: 'Galerie', ar: 'المعرض' })}
              </button>
            </div>
            <button
