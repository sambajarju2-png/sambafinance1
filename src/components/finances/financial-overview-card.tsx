    ...
    return (
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[14px] font-semibold text-pw-text">{t("setupTitle")}</p>
  ...
  return (
    <div className={`rounded-[14px] border p-4 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_2px_8px_rgba(0,0,0,0.04)] ${
      ...
      <div className="mb-3 flex items-start justify-between">
        <div>
          <p className="text-[11.5px] font-medium text-pw-muted">{t("disposable")}</p>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between text-[12px]">
          <span className="flex items-center gap-1.5 text-pw-muted">
            <Wallet className="h-3 w-3" /> {t("income")}
          </span>
          <span className="font-medium text-pw-text">{formatCents(data.totaal_inkomen)}</span>
        </div>
        <div className="flex items-center justify-between text-[12px]">
          <span className="flex items-center gap-1.5 text-pw-muted">
            <HomeIcon className="h-3 w-3" /> {t("fixedExpenses")} ({data.expenses_count})
          </span>
        ...
        {data.totaal_open_rekeningen > 0 && (
          <div className="flex items-center justify-between text-[12px]">
            <span className="flex items-center gap-1.5 text-pw-muted">
              <Receipt className="h-3 w-3" /> {t("openBills")} ({data.bills_count})
            </span>
        ...
        {data.totaal_betaald_deze_maand > 0 && (
          <div className="flex items-center justify-between text-[12px]">
            <span className="flex items-center gap-1.5 text-pw-muted">
              <Check className="h-3 w-3" /> {t("paidThisMonth")}
            </span>
      ...
      {data.expenses_in_incasso && data.expenses_in_incasso.length > 0 && (
        <div className="mt-3 rounded-lg bg-pw-red/[0.06] px-3 py-2">
          <p className="flex items-center gap-1 text-[11px] font-medium text-pw-red">
            ...
            {data.expenses_in_incasso.length} {data.expenses_in_incasso.length > 1 ? t("fixedInCollectionPlural") : t("fixedInCollection")}
          </p>
        </div>
      )}
      ...
      {data.salary_window && (
        <p className="mt-2 text-[10.5px] text-pw-muted">
          {t("salaryExpected")} {data.salary_window.from}–{data.salary_window.to}
        </p>
      )}
    </div>
  );
