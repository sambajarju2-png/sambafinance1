...
const LIMIT_INFO: Record<LimitType, { nl: LimitCopy; en: LimitCopy; pl: LimitCopy; tr: LimitCopy; fr: LimitCopy; ar: LimitCopy }> = {
  chat: {
  ...
  bank_account: {
    ...
    fr: { title: "Connexion de compte bancaire", desc: "La connexion de comptes bancaires est disponible à partir du forfait Pro.", icon: '🏦' },
    ar: { title: 'ربط حساب مصرفي', desc: 'ربط الحسابات المصرفية متاح بدءًا من خطة Pro.', icon: '🏦' },
  },
};
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={onClose}>
      ...
      >
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className={`text-[14px] font-semibold ${target.color}`}>
                {pick(lang, { nl: `Upgrade naar ${target.nameNl}`, en: `Upgrade to ${target.nameEn}`, pl: `Przejdź na ${target.nameNl}`, tr: `${target.nameEn}'a yükselt`, fr: `Passer à ${target.nameEn}`, ar: `الترقية إلى ${target.nameEn}` })}
              </p>
              <p className="text-[12px] text-gray-500 dark:text-gray-400">
                {pick(lang, { nl: 'Probeer 7 dagen gratis', en: '7-day free trial', pl: 'Wypróbuj 7 dni za darmo', tr: '7 gün ücretsiz dene', fr: 'Essai gratuit de 7 jours', ar: 'تجربة مجانية لمدة 7 أيام' })}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          {pick(lang, { nl: `Bekijk ${target.nameNl}-abonnement`, en: `View ${target.nameEn} plan`, pl: `Zobacz abonament ${target.nameNl}`, tr: `${target.nameEn} aboneliğini gör`, fr: `Voir le forfait ${target.nameEn}`, ar: `عرض خطة ${target.nameEn}` })}
        ...
        >
          {pick(lang, { nl: 'Niet nu', en: 'Not now', pl: 'Nie teraz', tr: 'Şimdi değil', fr: 'Pas maintenant', ar: 'ليس الآن' })}
        </button>
      </div>
    </div>
  );
