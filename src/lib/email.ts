...
export function buildWelcomeEmail(name: string, lang: string): { subject: string; html: string } {
  ...
  return {
    subject: E(lang, { nl: 'Je bent niet alleen (en wij snappen het)', en: "You're not alone (and we totally get it)", pl: 'Nie jesteś sam (i my to rozumiemy)', tr: 'Yalnız değilsin (ve bunu çok iyi anlıyoruz)', fr: "Tu n'es pas seul (et nous comprenons parfaitement)", ar: 'لست وحدك (ونحن نتفهّم ذلك تمامًا)' }),
    html: wrap(`
      ...
      <tr><td style="padding:16px 32px;font-size:14px;color:${C.navy};line-height:1.7">
        <p>${E(lang, {
          ...
          tr: "Paywatch'e hoş geldin. Sana söylemek istediğimiz ilk şey şu: <strong>nefes al, nefes ver. Artık bunda yalnız değilsin.</strong>", fr: "Bienvenue sur Paywatch. La première chose que nous voulons te dire est la suivante : <strong>inspire, expire. Tu n'es plus seul.</strong>", ar: 'مرحبًا بك في Paywatch. أول ما نريد قوله لك هو: <strong>خذ نفسًا، وأخرجه. لم تعد تواجه هذا وحدك.</strong>' })}</p>
        ${founderCard(lang)}
        <p>${E(lang, {
        ...
        <p>${E(lang, {
          ...
          tr: 'Hiç kimsenin posta kutusunu ya da gelen kutusunu açarken o huzursuzluğu yaşamasını istemiyoruz. Birlikte, o faturaların seni bir daha gafil avlamamasını sağlayacağız.', fr: 'Nous voulons que plus personne ne ressente cette boule au ventre en ouvrant sa boîte aux lettres ou sa boîte de réception. Ensemble, nous ferons en sorte que ces factures ne te surprennent plus.', ar: 'نريد ألّا يشعر أحد بعد الآن بتلك الغصّة عند فتح صندوق البريد أو البريد الإلكتروني. معًا، سنحرص على ألّا تباغتك تلك الفواتير مجددًا.' })}</p>
        ${zenBox(E(lang, { nl: '<strong>Jij houdt de regie, wij houden de wacht.</strong>', en: "<strong>You stay in control; we'll keep watch.</strong>", pl: '<strong>Ty masz kontrolę, my czuwamy.</strong>', tr: '<strong>Kontrol sende, nöbeti biz tutarız.</strong>', fr: '<strong>Tu gardes le contrôle ; nous montons la garde.</strong>', ar: '<strong>أنت تتحكّم، ونحن نراقب.</strong>' }))}
      </td></tr>
      <tr><td style="padding:0 32px 8px">${btn(E(lang, { nl: 'Start je eerste scan', en: 'Start your first scan', pl: 'Rozpocznij pierwsze skanowanie', tr: 'İlk taramanı başlat', fr: 'Lancer ton premier scan', ar: 'ابدأ أول فحص لك' }), 'https://app.paywatch.app/overzicht')}</td></tr>
      ...
      ${footerHtml(lang)}
    `, E(lang, { nl: 'Adem in, adem uit. Je staat er niet meer alleen voor.', en: "Breathe in, breathe out. You're not doing this alone anymore.", pl: 'Weź wdech, weź wydech. Nie jesteś już w tym sam.', tr: 'Nefes al, nefes ver. Artık bunda yalnız değilsin.', fr: "Inspire, expire. Tu n'es plus seul.", ar: 'خذ نفسًا، وأخرجه. لم تعد تواجه هذا وحدك.' }))
  };
}
...
export function buildFeaturesEmail(name: string, lang: string): { subject: string; html: string } {
  const firstName = name?.split(' ')[0] || '';
  return {
    subject: E(lang, { nl: 'Rust in je hoofd: zo werkt Paywatch', en: 'Peace of mind: how Paywatch works for you', pl: 'Spokój w głowie: tak działa Paywatch', tr: 'Kafan rahat olsun: Paywatch nasıl çalışır', fr: 'La sérénité : comment Paywatch travaille pour toi', ar: 'راحة البال: كيف يعمل Paywatch من أجلك' }),
    html: wrap(`
...
export function buildDigestEmail(name: string, lang: string, stats: {
  ...
  return {
    subject: E(lang, { nl: 'Jouw Paywatch Check-in: Lekker bezig!', en: "Your Paywatch Check-in: You've got this!", pl: 'Twoje podsumowanie Paywatch: Dobra robota!', tr: 'Paywatch özetin: Harika gidiyorsun!', fr: 'Ton point Paywatch : tu gères !', ar: 'متابعتك مع Paywatch: أنت قادر على ذلك!' }),
    html: wrap(`
      <tr><td style="padding:32px 32px 0;text-align:center"><p style="margin:0;font-size:12px;font-weight:800;color:${C.navy}">PayWatch</p></td></tr>
      <tr><td style="padding:20px 32px 0"><p style="margin:0;font-size:20px;font-weight:800;color:${C.navy}">${E(lang, { nl: `Hoi ${firstName}`, en: `Hey ${firstName}`, pl: `Cześć ${firstName}`, tr: `Merhaba ${firstName}` })}</p><p style="margin:6px 0 0;font-size:14px;color:${C.muted}">${E(lang, { nl: 'Tijd voor je wekelijkse momentje van rust. Even een korte blik op hoe je ervoor staat.', en: "Time for your weekly moment of calm. Just a quick look at where things stand.", pl: 'Czas na twoją cotygodniową chwilę spokoju. Krótkie spojrzenie na to, jak stoisz.', tr: 'Haftalık sakinlik anının zamanı. Durumuna kısaca bir göz atalım.', fr: "C'est l'heure de ton moment de calme hebdomadaire. Un rapide coup d'œil sur la situation.", ar: 'حان وقت لحظة الهدوء الأسبوعية. نظرة سريعة على وضعك.' })}</p></td></tr>
      <tr><td style="padding:16px 32px;font-size:14px;color:${C.navy};line-height:1.7">
      ...
      ${footerHtml(lang)}
      ${unsubscribeHtml}
    `, E(lang, { nl: 'Je wekelijkse overzicht staat klaar.', en: 'Your weekly overview is ready.', pl: 'Twoje cotygodniowe podsumowanie jest gotowe.', tr: 'Haftalık genel görünümün hazır.', fr: 'Ton aperçu hebdomadaire est prêt.', ar: 'ملخصك الأسبوعي جاهز.' }))
  };
}
